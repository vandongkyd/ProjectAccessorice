...
@section('title', 'Accessories | '.__('messages.lbl_screen_menu.edit',['name' => __('messages.lbl_screen_menu.product')]))

@section('content-heading')
    <h3 class="title">{{__('messages.lbl_screen_menu.product')}} <span>/ {{__('messages.lbl_header_edit')}}</span></h3>
@stop
        ...
        <div class="col-12 mb-30">
            <div class="box">
                ...
                <div class="col-lg-12 col-12 mb-30">
                    <div class="box-body">
                        <form id="f_submit" action="{{ route("product.submit.edit") }}" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="row mbn-20">
                                        ...
                                        <div class="col-12 mb-20">
                                            ...
                                            <select id="category" class="form-control select2" name="category">
                                                ...
                                                @foreach($categories as $category)
                                                    <option {{old("category",$product->category_id) == $category->id ? 'selected' : ''}} value="{{$category->id}}">{{$category->category_name}}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="row mbn-20">
                                        ...
                                        <div class="col-12 mb-20">
                                            ...
                                            <input type="number" id="product_quality" class="form-control"
                                                   name="product_quality" value="{{old('product_quality',$product->product_quality)}}"
                                        ...
                                        <div class="col-12 mb-20">
                                            ...
                                            <select id="discount" class="form-control select2" name="discount">
                                                ...
                                                @foreach($discounts as $discount)
                                                    <option {{old("discount",$product->discount_id) == $discount->id ? 'selected' : ''}} value="{{$discount->id}}">{{$discount->discount_name}}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="product-upload-gallery row flex-wrap">
                                        <div class="col-12 mb-30">
                                            ...
                                            <div id="image_preview">
                                                @if(count((array)$product_images) != 0)
                                                    @foreach($product_images as $product_image)
                                                        <div class="div-image-preview" data-imgname="{{$product_image->file_name}}" style="background-image: url({{asset('upload'.'/'. $product_image->file_name )}});">
                                                            <button type="button" class="btn-close-preview"><i class="fa fa-close"></i></button>
                                                            <input value="{{$product_image->sort_no}}" type="hidden" name="sort_no">
                                                            <input type="hidden" name="images_product[]" value="{{$product_image->file_name}}">
                                                        </div>
                                                    @endforeach
                                                @endif

                                                @if(count((array)old('images_product')) != 0 )
                                                    @for ($i = 0; $i < count((array)old('images_product')); $i++)
                                ...
                                <div class="col-lg-6 col-12 mb-30">
                                    ...
                                    <input type="text" class="form-control input-date-single" name="date_start" id="date_start"
                                           value="{{ date('m/d/Y', old('date_start') ? strtotime(old('date_start')) : $product->product_date_start )}}">
                                </div>
                                ...
                                <div class="col-lg-6 col-12 mb-30">
                                    ...
                                    <input type="text" class="form-control input-date-single" name="date_end" id="date_end"
                                           value="{{ date('m/d/Y', old('date_end') ? strtotime(old('date_end')) : $product->product_date_end )}}">
                                ...
                                <div class="col-12 mb-20">
                                    ...
                                    <textarea id="product_detail" name="product_detail" class="summernote_small">{{old('product_detail',$product->product_detail)}}</textarea>
                                ...
                                <div class="col-12 mb-20">
                                    ...
                                    <textarea id="product_description" name="product_description" class="summernote">{{old('product_description',$product->product_description)}}</textarea>
                                ...
                                <div class="col-6 mb-20">
                                    ...
                                    <select id="product_status" class="form-control" name="product_status">
                                        @foreach(__('messages.lbl_status_work') as $key => $value)
                                            <option {{old("product_status", $product->product_status) == $key ? 'selected' : ''}} value="{{$key}}">{{$value}}</option>
                                        @endforeach
                                    </select>
                                </div>
