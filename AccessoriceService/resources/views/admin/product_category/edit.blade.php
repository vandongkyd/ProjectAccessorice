...
@section('title', 'Accessories | '.__('messages.lbl_screen_menu.edit',['name' => __('messages.lbl_screen_menu.product_category')]))

@section('content-heading')
    <h3 class="title">{{__('messages.lbl_screen_menu.product_category')}} <span>/ {{__('messages.lbl_header_edit')}}</span></h3>
@stop
    ...
    <div class="row">
        <div class="col-12 mb-30">
            <div class="box">
                <div class="box-head">
                    <div class="row">
                        <h3 class="title col-lg-10 col-12">{{__('messages.lbl_screen_menu.edit',['name' => __('messages.lbl_screen_menu.product_category')])}}</h3>
                    </div>
                </div>

                <div class="col-lg-12 col-12 mb-30">
                    <div class="box-body">
                        <form id="f_submit" action="{{ route("product.category.submit.edit") }}" method="post" enctype="multipart/form-data">
                            @csrf
                                        ...
                                        <div class="col-12 mb-20">
                                            ...
                                            <select id="brand" class="form-control select2" name="brand">
                                                ...
                                                @foreach($brands as $brand)
                                                    <option {{old("brand",$category->brand_id) == $brand->id ? 'selected' : ''}} value="{{$brand->id}}">{{$brand->brand_name}}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="col-12 mb-20">
                                            ...
                                            <select id="discount" class="form-control select2" name="discount">
                                                ...
                                                @foreach($discounts as $discount)
                                                    <option {{old("discount",$category->discount_id) == $discount->id ? 'selected' : ''}} value="{{$discount->id}}">{{$discount->discount_name}}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="col-12 mb-20">
                                            ...
                                            <select id="category_status" class="form-control" name="category_status">
                                                @foreach(__('messages.lbl_status_work') as $key => $value)
                                                    <option {{old("category_status",$category->category_status) == $key ? 'selected' : ''}} value="{{$key}}">{{$value}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
