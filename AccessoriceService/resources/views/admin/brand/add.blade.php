...
@section('title', 'Accessories | '.__('messages.lbl_screen_menu.create',['name' => __('messages.lbl_screen_menu.brand')]))

@section('content-heading')
    <h3 class="title">{{__('messages.lbl_screen_menu.brand')}} <span>/ {{__('messages.lbl_header_add')}}</span></h3>
@stop
    ...
    <div class="row">
        <div class="col-12 mb-30">
            <div class="box">
                <div class="box-head">
                    <div class="row">
                        <h3 class="title col-lg-10 col-12">{{__('messages.lbl_screen_menu.create',['name' => __('messages.lbl_screen_menu.brand')])}}</h3>
                    </div>
                </div>

                <div class="col-lg-12 col-12 mb-30">
                    <div class="box-body">
                        <form id="f_submit" action="{{ route("brand.submit.add") }}" method="post" enctype="multipart/form-data">
                            @csrf
                                        ...
                                        <div class="col-12 mb-20">
                                            ...
                                            <select id="discount_id" class="form-control select2" name="discount_id">
                                                ...
                                                @foreach($discounts as $discount)
                                                    <option {{old("discount_name") == $discount->id ? 'selected' : ''}} value="{{$discount->id}}">{{$discount->discount_name}}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="col-12 mb-20">
                                            ...
                                            <select id="brand_status" class="form-control" name="brand_status">
                                                @foreach(__('messages.lbl_status_work') as $key => $value)
                                                    <option {{old("brand_status") == $key ? 'selected' : ''}} value="{{$key}}">{{$value}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                ...
                                <div class="col-lg-6 col-12 mb-30">
                                    <div class="row mbn-20">
                                        <div class="col-12 mb-20">
                                            ...
                                            <input type="file" id="brand_image" class="dropify" name="brand_image"
                                                   data-default-file="{{old("brand_image")}}" accept="image/*">
