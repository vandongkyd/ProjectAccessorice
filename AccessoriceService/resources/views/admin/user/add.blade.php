...
@section('title', 'Accessories | '.__('messages.lbl_screen_menu.create',['name' => __('messages.lbl_screen_menu.user')]))

@section('content-heading')
    <h3 class="title">{{__('messages.lbl_screen_menu.user')}} <span>/ {{__('messages.lbl_header_add')}}</span></h3>
@stop
    ...
    <div class="row">
        <div class="col-12 mb-30">
            <div class="box">
                <div class="box-head">
                    <div class="row">
                        <h3 class="title col-lg-10 col-12">{{__('messages.lbl_screen_menu.create',['name' => __('messages.lbl_screen_menu.user')])}}</h3>
                    </div>
                </div>

                <div class="col-lg-12 col-12 mb-30">
                    <div class="box-body">
                        <form id="f_submit" action="{{ route("user.submit.add") }}" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="row mbn-20">
                                        ...
                                        <div class="col-12 mb-20">
                                            ...
                                            <select id="shop_id" class="form-control" name="shop_id">
                                                ...
                                                @foreach($shops as $shop)
                                                    <option {{old("shop_id") == $shop->id ? 'selected' : ''}} value="{{$shop->id}}">{{$shop->shop_name}}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="col-12 mb-20">
                                            ...
                                            <select id="role_id" class="form-control" name="role_id">
                                                ...
                                                @foreach(__('messages.lbl_role_list') as $key => $value)
                                                    <option {{old("role_id") == $key ? 'selected' : ''}} value="{{$key}}">{{$value}}</option>
                                                @endforeach
                                            </select>
                                ...
                                <div class="col-12 mb-20">
                                    ...
                                    <textarea class="form-control" id="address" name="address" placeholder="{{__('messages.lbl_table_user.address')}}">{{old('address')}}</textarea>
                                    @error('address')
                                    <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
